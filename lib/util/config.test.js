...
describe('util/config', () => {
  ...
  describe('+ loadConfig(projectDir)', () => {
    ...
    function testWithConfig(name, callback) {
      test(name, () => {
        expect.assertions(1);
    ...
    describe('> when config file cannot be found', () => {
      testWithConfig('returns `null`', config => expect(config).toBeNull());
    });
    ...
    describe('> when config file is found', () => {
      ...
      describe('it returns', () => {
        testWithConfig('an object', config =>
          expect(typeof config).toBe('object')
        );

        testWithConfig('with config file path', ({ filePath }) =>
          expect(filePath).toBe(configFilePath)
        );

        testWithConfig('with config data', ({ data }) =>
